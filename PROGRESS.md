...
| Near-field wave contrast inverted | **open, partly improved, and the critic's numbers were unreliable** | Its sample (x600-700) straddles the sledgehammer viewmodel — roughly half those columns are flat-shaded orange, so the figure was largely measuring a viewmodel. Measured that region either way it reads "correct", so it cannot support the claim. **But the conclusion held on clean water** (x790-950): near/far 0.71 where it should exceed 1. Restoring the chop nearly doubled absolute detail (far 0.174 -> 0.317, near 0.123 -> 0.172) and the surface now visibly carries ripple where it was a smooth sheet, but it did **not** fix the ratio — 0.71 -> 0.54, slightly worse. Physical reason, which points at the real fix: near and steep means Fresnel ~0.02, so the pixel is almost entirely refraction of a smooth sea bed, and distorting a low-contrast image yields a low-contrast image. Far and grazing means high Fresnel over high-contrast lamps and structures, which the same distortion breaks up visibly. **Near-field structure has to come from specular glinting on the perturbed normal, not from distorting a featureless bed** — which is also the missing-glint delta, again. |
